...
class Sheet(BaseObject):
        ...
        while 1:
            ...
            if rc == XL_NUMBER:
                ...
                for colx in range(first_colx, last_colx + 1):
                    ...
                    if 0:
                        fprintf(self.logfile, "**COL %d %d %d\n", self.number, colx, c.xf_index)
                if blah:
                    fprintf(
                        self.logfile,
            ...
            #### all of the following are for BIFF <= 4W
                ...
                if rc == XL_FORMAT or rc == XL_FORMAT2:
                    ...
                    self_put_cell(
                        rowx, colx, None, d, self.fixed_BIFF2_xfindex(cell_attr, rowx, colx)
                    )
                elif rc == XL_INTEGER:
                    rowx, colx, cell_attr, d = local_unpack("<HH3sH", data)
                    self_put_cell(
                        rowx, colx, None, float(d), self.fixed_BIFF2_xfindex(cell_attr, rowx, colx)
                    )
                elif rc == XL_LABEL_B2:
                    ...
                    self_put_cell(
                        rowx, colx, cellty, value, self.fixed_BIFF2_xfindex(cell_attr, rowx, colx)
                    )
                elif rc == XL_BLANK_B2:
                    if not fmt_info:
                        continue
                    ...
                    if not (0 <= first_colx < last_colx <= 256):
                        print(
                            ...
                            "should have 0 <= first < last <= 256" % (first_colx, last_colx),
                            file=self.logfile,
                        )
                ...
                pass
        if not eof_found:
            raise XLRDError("Sheet %d (%r) missing EOF record" % (self.number, self.name))
    ...
    def handle_feat11(self, data):
        ...
        rt, grbitFrt, Ref0, isf, fHdr, reserved0, cref, cbFeatData, reserved1, Ref1 = unpack(
            "<HH8sHBiHiH8s", data[0:35]
        )
        assert reserved0 == 0
        assert reserved1 == 0
        assert isf == 5
