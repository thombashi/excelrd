...
class CompDoc(object):
    ...
    def __init__(self, mem, logfile=sys.stdout, DEBUG=0, ignore_workbook_corruption=False):
        ...
        if self.sec_size != 512 or self.short_sec_size != 64:
            print(
                "@@@@ sec_size=%d short_sec_size=%d" % (self.sec_size, self.short_sec_size),
                file=logfile,
            )
        (
            SAT_tot_secs,
        ...
        if size is None:
            ...
            while s >= 0:
                ...
                try:
                    ...
                    raise CompDocError(
                        "OLE2 stream %r: sector allocation table invalid entry (%d)" % (name, s)
                    )
            assert s == EOCSID
        else:
            todo = size
    ...
    def locate_named_stream(self, qname):
        ...
        if d.tot_size >= self.min_size_std_stream:
            result = self._locate_stream(
                self.mem, 512, self.SAT, self.sec_size, d.first_SID, d.tot_size, qname, d.DID + 6
            )
            if self.DEBUG:
