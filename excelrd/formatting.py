...
class Font(BaseObject, EqNeAttrs):
    ...
    #: Note that index 4 is never used; excelrd supplies a dummy place-holder.
    font_index = 0
...
class Format(BaseObject, EqNeAttrs):
    ...
    #: The key into :attr:`~excelrd.book.Book.format_map`
    format_key = 0
...
def handle_format(self, data, rectype=XL_FORMAT):
    ...
    if not (fmtkey > 163 or bv < 50):
        ...
        if self.verbosity and 0 < fmtkey < 50 and (is_date_c ^ is_date_s):
            ...
            fprintf(
                self.logfile,
                "WARNING *** Conflict between " "std format key %d and its format string %r\n",
                fmtkey,
                unistrg,
            )
    if DEBUG == 2:
...
def handle_palette(book, data):
    ...
    if not expected_size <= actual_size <= expected_size + tolerance:
        raise XLRDError(
            "PALETTE record: expected size %d, actual size %d" % (expected_size, actual_size)
        )
...
def check_colour_indexes_in_obj(book, obj, orig_index):
    ...
    for attr, nobj in alist:
        if hasattr(nobj, "dump"):
            ...
            print(
                "*** xf #%d : %s.%s =  0x%04x (unknown)" % (orig_index, oname, attr, nobj),
                file=book.logfile,
            )
...
def fill_in_standard_formats(book):
    for x in std_format_code_types.keys():
        if x not in book.format_map:
            ...
            # format strings that vary by locale; excelrd does not (yet)
...
class XF(BaseObject):
    ...
    #: Index into :attr:`~excelrd.book.Book.xf_list`
    xf_index = 0

    #: Index into :attr:`~excelrd.book.Book.font_list`
    font_index = 0

    #: Key into :attr:`~excelrd.book.Book.format_map`
    #:
    #: .. warning::
